        ...
        if sx >= 0 and sy >= 0 and sx < sw and sy < sh then
            ...
            if result and colpoint.entity ~= 0 then
                ...
                if result then
                    ...
                    sy = sy - 2
                    sx = sx - 2
                    --renderFontDrawText(font, string.format("%0.2fì", dist), sx, sy - hoffs, 0xEEEEEEEEE)
                    local tpIntoCar = nil
                    if colpoint.entityType == 2 then
                        local car = getVehiclePointerHandle(colpoint.entity)
                        local res, idcar =  sampGetVehicleIdByCarHandle(car)
                        if doesVehicleExist(car) and (not isCharInAnyCar(playerPed) or storeCarCharIsInNoSave(playerPed) ~= car) then
                            renderFontDrawText(font2, string.format("ID: %d, %0.2fm\nÑåñòü: ÏÊÌ + ËÊÌ\nÒåëåïîðòèðîâàòü: ALT + ÏÊÌ", idcar, dist), sx, sy - hoffs, 0xEEEEEEEEE)
                            renderFontDrawText(font, getNameOfVehicleModel(getCarModel(car)), sx, sy - hoffs * 2, 0xEEEEEEEE)
                    ...
                    if isKeyDown(keyApply) then
                        ...
                        showCursor(false)
                    elseif isKeyDown(18) and isKeyJustPressed(02) then
                        local car = getVehiclePointerHandle(colpoint.entity)
                        local res, idcar =  sampGetVehicleIdByCarHandle(car)
                        if idcar >= 0 then
                            sampSendChat('/getherecar '..idcar)
                            removePointMarker()
                            showCursor(false)
                        end
                    end
...
function initializeRender()
    font = renderCreateFont("Comic Sans MS", 10, FCR_BOLD + FCR_BORDER)
    font2 = renderCreateFont("Calibri", 9, FCR_ITALICS + FCR_BORDER)
end
